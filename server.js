...
async function getQuotaStatus(email) {
  ...
  // encontra o próximo mês por pagar
  let nextUnpaid = startYM;
  while (paidSet.has(nextUnpaid)) nextUnpaid++;

  // “em atraso”: meses por pagar até ao mês atual
  const overdueMonths = Math.max(0, (nowYM - nextUnpaid) + 1);

  // validade: se está tudo pago até ao mês atual, calcula até quando está pago
  let paidUntil = startYM - 1;
  while (paidSet.has(paidUntil + 1)) paidUntil++;

  // dias restantes: até ao fim do mês
  let diasRestantes = 0;
...
const loginLimiter = rateLimit({
  ...
  max: 7, // Limite de 7 tentativas por IP em 15 minutos( que estão defenidos na linha acima )
...
async function ensureGamesSeeded() {
  ...
  console.log("✅ Jogos inseridos na Base de dados");
}
...
const userSchema = new mongoose.Schema(
  {
    ...
    isVerified: { type: Boolean, default: false }, // Começa falso porque o email do utilizador não começa verificado
...
app.post("/api/registar", async (req, res) => {
  try {
    ...
    // Código Temporário (para email) - 6 dígitos
    ...
    // Enviar o código para email real
    try {
...
app.post("/api/login", loginLimiter, async (req, res) => {
  try {
    ...
    // Verificamos a password PRIMEIRO
    const ok = await bcrypt.compare(password.trim(), user.passwordHash);
    if (!ok) return res.status(401).json({ error: "Credenciais inválidas." });

    // Se a password está certa, mas a conta NÃO está verificada
    if (!user.isVerified) {
      ...
      // Enviar o novo código para email real
      try {
...
// RECUPERAR PASSWORD: Enviar código para o email do utilizador
app.post("/api/recuperar-password", rateLimit({
  ...
  try {
    ...
    const user = await User.findOne({ email });
    // Por segurança, não se revela se o email existe ou não
    if (!user) {
...
app.post("/api/conta/mudar-password", async (req, res) => {
  try {
    ...
    // impedir repetir a mesma password de antes
...
app.post("/api/bilhetes/comprar", async (req, res) => {
  try {
    ...
    // VIP só Premium com numero sócio 1-499
    if (setor === "Zona VIP") {
...
app.post("/api/inscricoes/criar", async (req, res) => {
  try {
    ...
    // numero sócio vem sempre do utilizador
...
app.post("/api/conta/apagar/pedir", async (req, res) => {
  try {
    ...
    user.deleteAccountCodeExpiry = new Date(Date.now() + 15 * 60 * 1000); // 15 minutos ate expirar o codigo
    await user.save();
  ...
  try {
    ...
    try {
      await transporter.sendMail({
        from: `"Esfera" <${process.env.EMAIL_USER}>`,
        to: email,
        subject: "A sua conta foi eliminada — Esfera",
        html: `
          <div style="font-family: Arial, sans-serif; max-width: 480px; margin: 0 auto; padding: 20px;">
